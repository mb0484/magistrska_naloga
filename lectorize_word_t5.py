...
from sloBertCommaPredictor import markUpWord
...
masks_model = T5ForConditionalGeneration.from_pretrained("./maksed_model_aug_22/")
masks_tokenizer = T5Tokenizer.from_pretrained("./maksed_model_aug_22/")
...
def correct_word_mistakes(input_text, markUp=False, sklanjaj=False, wo=False, names=False, indexses_markup_vejice=[], just_predictions=False):
    ...
    for section in sections:
        new_section, napaka = correct_word_mistakes_big_sections(section, markUp, sklanjaj, wo, names, indexses_markup_vejice, just_predictions)
...
def correct_word_mistakes_big_sections(cur_section, markUp=False, sklanjaj=False, wo=False, names=True, indexses_markup_vejice=[], just_predictions=False):
    final_text = ""
    ...
    while (len(cur_section) > 0):
        ...
        if wo:
            ...
            new_section, napaka = correct_word_mistakes_section(new_section, markUp, sklanjaj, names, indexses_markup_vejice, just_predictions)
...
def correct_word_mistakes_section(input_text, markUp=False, sklanjaj=False, names=True, indexses_markup_vejice=[], just_predictions=False):
    global predictionNum
    
    predictions_for_return = []
    ...
    while has_masks:
        ...
        #print("masked_result_split2: ", masked_result_split)
        ...
        for idx, (predicted_word, word) in enumerate(zip(masked_result_split, input_split)):
            ...
            if (predicted_word == '<mask>') and (len(word) > 1 or word.isalpha()) and not idx in idxesToExclude:
                ...
                for i, beam_output in enumerate(beam_outputs):
                    ...
                    if (len(predicted_word.split(' ')) > 0 and predicted_word[0] in sloleks_root_dict) or ((predicted_word in sloleks_root_dict or predicted_word.lower() in sloleks_root_dict) and (predicted_word in root_lemma_dict or predicted_word.lower() in root_lemma_dict)):
                        ...
                        if not shouldBreak:
                            ...
                            if i < 5 and calculated_lavenshtein_distance == 2 and prediction_2['position'] == -1:
                                prediction_2['word'] = predicted_word
                                
                                prediction_2['position'] = generatedTextIdx
                                    
                            if i < 2 and calculated_lavenshtein_distance == 3 and prediction_3['position'] == -1:
                ...
                predictions_for_return.append(prediction)
    ...
    for idx, (word, orig_word) in enumerate(zip(input_text.split(' '), input_split)):
        ...
        if word != orig_word:
            orthr_return_predictions.append(word)
        
        if idx in predictions_all:
            
            if predictions_all[idx]['prediction'] != orig_word:
                prediction = "<mark class='markPopravljenaCrkovnaNapaka'>" + addOptions(predictions_all[idx]['othrPredictions'], predictions_all[idx]['prediction'], predictions_all[idx]['predictionNum']) + "</mark>"
            else:
                prediction = "<mark class='markNePopravljenaCrkovnaNapaka'>" + addOptions(predictions_all[idx]['othrPredictions'], predictions_all[idx]['prediction'], predictions_all[idx]['predictionNum']) + "</mark>"
        ...
        elif idx < len(indexses_markup_vejice) and indexses_markup_vejice[idx] == 1:
            prediction = "<mark class='markVejicaPrejJaZdejNe'>" + word + "</mark>"
            
            predictionNum += 1
            
        elif idx < len(indexses_markup_vejice) and indexses_markup_vejice[idx] == 2:
            prediction = "<mark class='markVejicaPrejNeZdejJa'>" + word + "</mark>"
    ...
    if just_predictions:
        return (predictions_for_return, " ".join(masked_result_split), orthr_return_predictions)
    return (input_text, 0)
